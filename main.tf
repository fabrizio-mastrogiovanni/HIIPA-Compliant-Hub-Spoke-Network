...
resource "azurerm_security_center_subscription_pricing" "defender_keyvault" {
  ...
  depends_on    = [azurerm_security_center_subscription_pricing.defender_storage]
}

# Assign the HIPAA built-in policy initiative to the subscription
...
# ── NSG Flow Logs ─────────────────────────────────────────────────────
# Requires Network Watcher in the same region
resource "azurerm_network_watcher" "main" {
  name                = "nw-hipaa-${var.yourname}"
  location            = azurerm_resource_group.hub.location
  resource_group_name = azurerm_resource_group.hub.name
  tags                = var.tags
}

# ── VNet Flow Logs (replaces retired NSG flow logs) ──────────────────
...
resource "azurerm_network_watcher_flow_log" "spoke2_vnet" {
